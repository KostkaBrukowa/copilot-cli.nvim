local config = require("copilot_cli.config")
...
describe("Auto Reload Feature", function()
  ...
  before_each(function()
    ...
    pcall(vim.api.nvim_del_augroup_by_name, "CopilotAutoRefresh")
  end)
  ...
  after_each(function()
    ...
    pcall(vim.api.nvim_del_augroup_by_name, "CopilotAutoRefresh")
    package.loaded["copilot_cli"] = nil -- Force reload if needed
    package.loaded["copilot_cli.config"] = nil -- Force config reload
  end)
  ...
  --   vim.o.autoread = true
  --   copilot_cli.setup({ auto_reload = true })
  --
  ...
  --   vim.o.autoread = true
  --   copilot_cli.setup({ auto_reload = false })
  --
  --   -- Simulate external file change
  --   vim.fn.writefile({ "modified content" }, temp_file)
  --
  ...
  it("should NOT trigger checktime when autoread is false", function()
    ...
    local groups = vim.api.nvim_get_augroup_by_name("CopilotAutoRefresh")
  ...
  it("should show notification if autoread is false but auto_reload is true", function()
    ...
    local expected_start = "    local expected_start = "nvim‑copilot-cli: auto‑reload disabled"" -- NOTE: These are non-breaking hyphens
    assert.truthy(
